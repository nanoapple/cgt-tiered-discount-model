...
# =============================================================================
# Each variable is standardised to its truncated percentile rank WITHIN the
# investor population, putting all variables on a common [0,1] scale.
...
ato_data <- ato_data %>%
  mutate(
    ...
    S_hat = ifelse(supply_contribution == 1, 0.20, 0),
    H_hat = ifelse(hardship_indicator  == 1, 0.20, 0)
  )

# =============================================================================
# 2. Composite investor index F
# =============================================================================
# F = w_I·Î + w_L·L̂ + w_A·Â + w_V·V̂ − w_t·t̂ − w_S·Ŝ − w_H·Ĥ
#
# Positive weights (increase F → less discount):
#   I=income, L=leverage, A=portfolio count, V=property value
# Negative weights (decrease F → more discount):
#   t=holding period, S=supply contribution, H=hardship

w <- c(I=0.30, L=0.20, A=0.15, V=0.15, t=0.12, S=0.05, H=0.03)
cat("=== Composite index weights ===\n")
cat(paste(sprintf("  %s = %.2f", names(w), w), collapse="\n"), "\n\n")

ato_data <- ato_data %>%
  mutate(
    F_raw = w["I"]*I_hat + w["L"]*L_hat + w["A"]*A_hat + w["V"]*V_hat
            - w["t"]*t_hat - w["S"]*S_hat - w["H"]*H_hat,
    F = pmin(pmax(F_raw, 0), 1)
  )
